...
namespace subs2srs
{
  public partial class DialogPref : Form
  {
    ...
    public DialogPref()
    {
      ...
      // abc123sm 音频
      propTable.Properties.Add(new PropertySpec("音频线程数", typeof(int),
        "ABC123SM (Uses Tokens)",
        "设定转换音频时使用的线程数（分母），设置为1则使用所有线程核，设为4则使用1/4个线程核\n如10核20线程的cpu设置4，则使用5个线程核进行任务\n",
        PrefDefaults.Defaultabc123smaudiocpu));
      propTable["音频线程数"] = ConstantSettings.Defaultabc123smaudiocpu;
      // abc123sm 图片
      propTable.Properties.Add(new PropertySpec("截图线程数", typeof(int),
        "ABC123SM (Uses Tokens)",
        "设定截图时使用的线程数（分母），设置为1则使用所有线程核，设为4则使用1 / 4个线程核\n如10核20线程的cpu设置4，则使用5个线程核进行任务\n",
      ...
      propTable.Properties.Add(new PropertySpec("Normalize Audio Arguments", typeof(string),
        "Misc",
        "The arguments to pass to rsgain (the tool used to normalize the audio).\n"
