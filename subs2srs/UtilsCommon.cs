...
namespace subs2srs
{
  ...
  public class UtilsCommon
  {
    ...
    /// </summary>
    static private bool callExe(string exe, string args, bool useShellExecute, bool createNoWindow, string workingDirectory = null) // 添加 workingDirectory 参数
    {
    ...
    static public void startProcess(string relExePath, string fullExePath, string args,
      bool useShellExecute, bool createNoWindow, string workingDirectory = null) // 添加 workingDirectory 参数
    {
      Process process = new Process();
      bool status = true;
      ...
      status = callExe(relExePath, args, useShellExecute, createNoWindow, workingDirectory); // 传递 workingDirectory

      // Try absolute path to exe
      if (!status)
      {
        status = callExe(fullExePath, args, useShellExecute, createNoWindow, workingDirectory); // 传递 workingDirectory
      }
      ...
      if (!status)
      {
        ...
        status = callExe(Path.GetFileName(fullExePath), args, useShellExecute, createNoWindow, workingDirectory); // 传递 workingDirectory
      }
    }
    ...
    static public void startProcess(string relExePath, string fullExePath, string args)
    {
      startProcess(relExePath, fullExePath, args, false, true, null); // 最后一个参数传 null
    }


    /// <summary>
    /// **新增方法：** 带工作目录的 startProcess 简化调用
    /// </summary>
    static public void startProcess(string relExePath, string fullExePath, string args, string workingDirectory)
    {
        startProcess(relExePath, fullExePath, args, false, true, workingDirectory);
    }
