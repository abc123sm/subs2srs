      ...
      Profilex264 profile, Presetx264 preset, string outFile, DialogProgress dialogProgress)
        {
            ...
            // 修改为使用 Intel Quick Sync Video 编码器
            string videoCodecArg = "-c:v hevc_qsv"; // 使用 Intel QSV 作为视频编码器
            string hwaccelArg = "-hwaccel auto";

            string presetArg = ""; // QSV 预设不需要显式设置
            string keyframeOptionsArg = "";//UtilsVideo.formatKeyframeOptionsArg(videoCodec);
            string profileArg = ""; // QSV 可能不需要显式设置配置文件
            ...
            string videoBitrateArg = String.Format("");//不设置码率
            ...
            // 复制
            string ffmpegConvertArgs = String.Format("-y -i \"{0}\" {1} {2} -c copy" +
                                                      " {10} \"{14}\" ",
                                                      inFile,                   // {0}
            ...
            /* 使用核显
            string ffmpegConvertArgs = String.Format("-y {15} -i \"{0}\" -ac 2 {1} {2} {3} {7} {8} {9}" +
                                                      " {10} \"{14}\" ",
                                                      inFile,                   // {0}
